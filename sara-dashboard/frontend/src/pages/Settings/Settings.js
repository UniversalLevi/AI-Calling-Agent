...
const Settings = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        // Load voice settings
        const voiceRes = await axios.get('/system/voice-settings');
        if (voiceRes.data.success) {
          setVoiceSettings(prev => ({ ...prev, ...voiceRes.data.data }));
        }
        
        // Load humanization settings
        const humanizationRes = await axios.get('/system/humanization-settings');
            ...
            <div>
              <button
                onClick={async () => {
                  setSaving(true);
                  try {
                    await axios.put('/system/voice-settings', voiceSettings);
            ...
            <div>
              <button
                onClick={async () => {
                  setSaving(true);
                  try {
                    await axios.put('/system/humanization-settings', voiceSettings);
